"""
Standalone background worker for Opera.
...
def _acquire_file_lock():
    lock_path = os.getenv("OPERA_WORKER_LOCK_FILE", "/tmp/opera-worker.lock")
    handle = open(lock_path, "w", encoding="utf-8")
    try:
        fcntl.flock(handle, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError:
        handle.close()
        logger.warning("Another Opera worker is already running; lock_file=%s", lock_path)
        return None
    handle.seek(0)
    handle.truncate()
...
async def main() -> None:
    for stream in (sys.stdout, sys.stderr):
        try:
            ...
            pass
    try:
        os.nice(int(os.getenv("OPERA_WORKER_NICE", "10")))
    except Exception:
        pass
    ...
    try:
        lock_timeout_seconds = max(30, int(os.getenv("OPERA_WORKER_LOCK_TIMEOUT_SECONDS", "120")))
    except Exception:
    ...
    try:
        ...
        if redis_lock is not None:
            ...
            if not acquired:
                logger.warning("Another Opera worker is already running; Redis singleton lock is held.")
                return
            lock_renew_task = asyncio.create_task(
                _renew_redis_lock(redis_lock, lock_timeout_seconds),
                name="opera:worker_lock_renew",
            )
    ...
    try:
        ...
        if os.getenv("OPERA_BACKGROUND_TASKS") is None:
            os.environ["OPERA_BACKGROUND_TASKS"] = DEFAULT_BACKGROUND_TASKS

        tasks = start_background_tasks(logger)
        if not tasks:
            logger.warning("No background tasks enabled; set OPERA_BACKGROUND_TASKS to a comma-separated task list.")
            return
        ...
        if os.getenv("PROFIT_HISTORY_PRUNE_ON_WORKER_START", "false").strip().lower() in {"1", "true", "yes", "on"}:
            ...
            tasks.append(asyncio.create_task(asyncio.to_thread(_prune_profit_history), name="opera:startup_profit_history_prune"))
