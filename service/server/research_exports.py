...
CHALLENGE_EXPORTS: dict[str, dict[str, Any]] = {
    'challenges.csv': {
        ...
        'columns': [
            ...
            'challenge_type', 'mode', 'status', 'scoring_method', 'initial_capital',
...
HASH_SALT = _os.getenv("RESEARCH_EXPORT_HASH_SALT", "opera-research-v1")
...
CHALLENGE_DATASETS = [
    ...
    "challenge_results.csv",
    "challenge_teams.csv",
    "challenge_team_members.csv",
    "challenge_team_trades.csv",
    "challenge_team_submissions.csv",
    "challenge_submission_votes.csv",
]
TEAM_DATASETS = [
    "team_missions.csv",
    "teams.csv",
...
RESEARCH_EXPORTS = {
    ...
    "challenges.csv": {
        ...
        "selects": [
            ...
            ("challenge_type", "c.challenge_type"), ("mode", "c.mode"), ("status", "c.status"),
    ...
    "challenge_teams.csv": {
        ...
        "alias": "ctm",
        "join": "JOIN challenges c ON c.id = ctm.challenge_id",
        "selects": [
            ("id", "ctm.id"), ("challenge_id", "ctm.challenge_id"), ("challenge_key", "c.challenge_key"),
            ("team_key", "ctm.team_key"), ("name", "ctm.name"), ("status", "ctm.status"),
            ("variant_key", "ctm.variant_key"), ("created_by_agent_id", "ctm.created_by_agent_id"),
            ("created_by_agent_hash", "ctm.created_by_agent_id"), ("starting_cash", "ctm.starting_cash"),
            ("ending_value", "ctm.ending_value"), ("return_pct", "ctm.return_pct"),
            ("max_drawdown", "ctm.max_drawdown"), ("trade_count", "ctm.trade_count"),
            ("rank", "ctm.rank"), ("disqualified_reason", "ctm.disqualified_reason"),
            ("experiment_key", "c.experiment_key"), ("market", "c.market"),
            ("created_at", "ctm.created_at"), ("updated_at", "ctm.updated_at"),
        ],
        "time_column": "ctm.created_at",
        "agent_id_columns": ["ctm.created_by_agent_id"],
        "market_column": "c.market",
        "experiment_column": "c.experiment_key",
        "variant_column": "ctm.variant_key",
        "challenge_key_column": "c.challenge_key",
        "order_by": "ctm.id",
    },
    "challenge_team_members.csv": {
        "table": "challenge_team_members",
        "alias": "ctmem",
        "join": "JOIN challenges c ON c.id = ctmem.challenge_id JOIN challenge_teams ctm ON ctm.id = ctmem.team_id",
        "selects": [
            ("id", "ctmem.id"), ("challenge_id", "ctmem.challenge_id"), ("challenge_key", "c.challenge_key"),
            ("team_id", "ctmem.team_id"), ("team_key", "ctm.team_key"),
            ("agent_id", "ctmem.agent_id"), ("agent_hash", "ctmem.agent_id"),
            ("role", "ctmem.role"), ("status", "ctmem.status"), ("variant_key", "ctmem.variant_key"),
            ("experiment_key", "c.experiment_key"), ("market", "c.market"), ("joined_at", "ctmem.joined_at"),
        ],
        "time_column": "ctmem.joined_at",
        "agent_id_columns": ["ctmem.agent_id"],
        "market_column": "c.market",
        "experiment_column": "c.experiment_key",
        "variant_column": "ctmem.variant_key",
        "challenge_key_column": "c.challenge_key",
        "order_by": "ctmem.id",
    },
    "challenge_team_trades.csv": {
        "table": "challenge_team_trades",
        "alias": "ctt",
        "join": "JOIN challenges c ON c.id = ctt.challenge_id JOIN challenge_teams ctm ON ctm.id = ctt.team_id LEFT JOIN challenge_team_members ctmem ON ctmem.team_id = ctt.team_id AND ctmem.agent_id = ctt.agent_id",
        "selects": [
            ("id", "ctt.id"), ("challenge_id", "ctt.challenge_id"), ("challenge_key", "c.challenge_key"),
            ("team_id", "ctt.team_id"), ("team_key", "ctm.team_key"),
            ("agent_id", "ctt.agent_id"), ("agent_hash", "ctt.agent_id"),
            ("market", "ctt.market"), ("symbol", "ctt.symbol"), ("token_id", "ctt.token_id"),
            ("outcome", "ctt.outcome"), ("side", "ctt.side"), ("price", "ctt.price"),
            ("quantity", "ctt.quantity"), ("executed_at", "ctt.executed_at"),
            ("created_at", "ctt.created_at"), ("experiment_key", "c.experiment_key"),
            ("variant_key", "ctmem.variant_key"),
        ],
        "time_column": "ctt.executed_at",
        "agent_id_columns": ["ctt.agent_id"],
        "market_column": "ctt.market",
        "experiment_column": "c.experiment_key",
        "variant_column": "ctmem.variant_key",
        "challenge_key_column": "c.challenge_key",
        "order_by": "ctt.id",
    ...
    "challenge_team_submissions.csv": {
        "table": "challenge_team_submissions",
        "alias": "cts",
        "join": "JOIN challenges c ON c.id = cts.challenge_id JOIN challenge_teams ctm ON ctm.id = cts.team_id LEFT JOIN challenge_team_members ctmem ON ctmem.team_id = cts.team_id AND ctmem.agent_id = cts.agent_id",
        "selects": [
            ("id", "cts.id"), ("challenge_id", "cts.challenge_id"), ("challenge_key", "c.challenge_key"),
            ("team_id", "cts.team_id"), ("team_key", "ctm.team_key"),
            ("agent_id", "cts.agent_id"), ("agent_hash", "cts.agent_id"),
            ("submission_type", "cts.submission_type"), ("content", "cts.content"),
            ("prediction_json", "cts.prediction_json"), ("created_at", "cts.created_at"),
            ("experiment_key", "c.experiment_key"), ("variant_key", "ctmem.variant_key"),
            ("market", "c.market"),
        ],
        "time_column": "cts.created_at",
        "agent_id_columns": ["cts.agent_id"],
        "market_column": "c.market",
        "experiment_column": "c.experiment_key",
        "variant_column": "ctmem.variant_key",
        "challenge_key_column": "c.challenge_key",
    ...
    "challenge_submission_votes.csv": {
        "table": "challenge_submission_votes",
        "alias": "csvote",
        "join": "JOIN challenge_team_submissions cts ON cts.id = csvote.submission_id JOIN challenges c ON c.id = cts.challenge_id JOIN challenge_teams ctm ON ctm.id = cts.team_id LEFT JOIN challenge_team_members ctmem ON ctmem.team_id = cts.team_id AND ctmem.agent_id = csvote.agent_id",
        "selects": [
            ("id", "csvote.id"), ("submission_id", "csvote.submission_id"),
            ("challenge_id", "cts.challenge_id"), ("challenge_key", "c.challenge_key"),
            ("team_id", "cts.team_id"), ("team_key", "ctm.team_key"),
            ("agent_id", "csvote.agent_id"), ("agent_hash", "csvote.agent_id"),
            ("vote", "csvote.vote"), ("content", "csvote.content"),
            ("experiment_key", "c.experiment_key"), ("variant_key", "ctmem.variant_key"),
            ("market", "c.market"), ("created_at", "csvote.created_at"), ("updated_at", "csvote.updated_at"),
        ],
        "time_column": "csvote.created_at",
        "agent_id_columns": ["csvote.agent_id"],
        "market_column": "c.market",
        "experiment_column": "c.experiment_key",
        "variant_column": "ctmem.variant_key",
        "challenge_key_column": "c.challenge_key",
...
def research_schema_for_dataset(dataset_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "$id": f"https://opera.local/research/schemas/{filename[:-4]}.schema.json",
